...
///----------------------------------------
    ...
    size_t m_n_retry = 0;
    uint64_t m_seed{uint64_t(uintptr_t(&m_seed))};
    Stat m_stat = {0,0,0,0};
    ...
    ~LazyFreeListTLS();
    void on_retry_cnt(size_t retry_cnt) {
        m_n_retry++;
        if (retry_cnt >= 8) {
            if (retry_cnt >= 64) {
                auto max_dur_us = (retry_cnt / 16) % 997 + 1;
                auto dur = std::chrono::microseconds(xorshift64star(m_seed) % max_dur_us);
...
void PatriciaMem<Align>::mempool_set_readonly() {
  if (m_is_virtual_alloc && mmap_base) {
    ...
    if (ftruncate(m_fd, realsize) != 0) {
        THROW_STD(runtime_error, "ftruncate(realsize=%zd) failed: %s",
