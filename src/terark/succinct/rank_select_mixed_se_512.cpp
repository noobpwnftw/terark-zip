...
void rank_select_mixed_se_512::grow() noexcept {
    ...
    size_t newcapBytes = 2 * std::max(m_capacity, size_t(WordBits));
    bm_uint_t* new_words = (bm_uint_t*)realloc(m_words, newcapBytes/8);
    TERARK_VERIFY_F(nullptr != new_words, "newcapBytes = %zd", newcapBytes);
    if (g_Terark_hasValgrind) {
        byte_t* q = (byte_t*)new_words;
        memset(q + m_capacity/8, 0, (newcapBytes - m_capacity)/8);
    }
    m_words = new_words;
    m_capacity = newcapBytes;
}

void rank_select_mixed_se_512::reserve(size_t newcapBytes) {
    assert(align_up(newcapBytes, WordBits) == newcapBytes);
    if (newcapBytes <= m_capacity)
        return;
    auto new_words = (bm_uint_t*)realloc(m_words, newcapBytes / 8);
    if (NULL == new_words)
        throw std::bad_alloc();
    if (g_Terark_hasValgrind) {
        byte_t* q = (byte_t*)new_words;
